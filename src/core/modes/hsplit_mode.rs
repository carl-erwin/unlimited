...
pub fn hsplit_input_event(editor: &mut Editor, env: &mut EditorEnv, view: &Rc<RefCell<View>>) {
    ...
    if let Some(pvid) = v.parent_id {
        ...
        dbg_println!("VSPLIT SCREEN HEIGHT  = {}", env.height);
...
impl Filter<'_> for HsplitModeComposeFilter {
    fn name(&self) -> &'static str {
