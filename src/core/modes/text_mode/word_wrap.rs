        ...
        for io in filter_in.iter() {
            ...
            } = &*io
            {
                ...
                if self.column_count > self.max_column {
                    eprintln!("line is full");

                    // blank ?
                    if c == ' ' || c == '\n' {
                        eprintln!("last char is blank, ignore wrapping");
                        self.column_count = 0;
            ...
            for io in filter_in.iter() {
                ...
                } = &*io
                {
                    ...
                    if self.column_count == self.max_column {
                        ...
                        // look for first space backward, and split
                        let mut blank_idx = 0; // option ?
                        let mut blank_offset = None; // option ?
                    ...
                    match c {
                        ...
                        /*
                        // handle space on last column
                        ' ' if self.column_count + 1 >= self.max_column => {
                            // flush accum
                            filter_out.append(&mut self.accum);
